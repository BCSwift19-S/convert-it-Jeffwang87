...
class ViewController: UIViewController {
    ...
    let formulasArray = [Formula(conversionString: "miles to kilometers", formula: {$0/0.62137}),
                         ...
                         Formula(conversionString: "fahreheit to celsius", formula: {($0-32) * (5/9)}),
    ...
    func calaculateConversion() {
            guard let inputValue = Double(userInput.text!)else{
            if userInput.text! !=  ""{
             showAlert(tile: "Cannot Convert Value", message: "\"\(userInput.text!)\" is not a valid number.")
        }
            return
        }
